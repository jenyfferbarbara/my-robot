...
def buy_digital(line, payout, recovery_value = None, gale = False):
	...
	if(buys_status):
		...
		buy_gale(line, payout, entry_value, buy_digital, gale)
		while True:
			status,result = API.check_win_digital_v2(id)
			if status:
				update_results(line, result, recovery_value, gale)
				retry_gale(line, payout, entry_value, buy_digital, gale, result)
				break
			time.sleep(1)
	else :
...
def buy_binaria(line, payout, recovery_value = None, gale = False):
	...
	if(status):	
		update_status(line, "Processing")	
		buy_gale(line, payout, entry_value, buy_binaria, gale)		
		result = API.check_win_v3(id)
		update_results(line, result[1], recovery_value, gale)
		retry_gale(line, payout, entry_value, buy_binaria, gale, result[1])
	else:
		update_status(line, "Fail Bin")
		buy_digital(line, payout)

def buy_gale(line, payout, entry_value, option, gale):
	...
	if not gale:
		...
		if check_gale(par, timeframe, action, entry_time):
			update_status(line, "LOSS - check")
			...
			update_status(line, "WIN - check")

def retry_gale(line, payout, entry_value, option, gale, result):

	status = get_signal(line)["signal"]["status"] 
	channel = get_channel(line["channel"])

	if result < 0 and status != ["Processing"] and status != ["Done"] and not gale and channel["gale"] == True:
		job_thread = threading.Thread(target=option, args=[line, payout, entry_value, True])
		job_thread.start()

def check_gale(par, timeframe, action, entry_time):

	check_time = get_check_time(timeframe, entry_time)

	while True:
		now = datetime.now().strftime('%H:%M:%S')
		if(now == check_time):
			result = check_win_current(par, timeframe, action)
			log.info(f"check_gale: {result}")
			return True if result == "LOSS" else False
		time.sleep(1)

def check_win_current(par, timeframe, action):
